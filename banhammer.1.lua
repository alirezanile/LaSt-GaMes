...
local function username_id(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if vusername == member then
      ...
      elseif get_cmd == 'عن' then
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] unbanned')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'User '..user_id..' unbanned'
      elseif get_cmd == 'اخراج کلی' then
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] globally banned')
        return banall_user(member_id, chat_id)
      elseif get_cmd == 'اخراج نیست' then
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] unbanned')
...
local function run(msg, matches)
 if matches[1]:lower() == 'ایدی' then
  ...
  if matches[1]:lower() == 'سیک' then-- /kickme
    if msg.to.type == 'chat' then
      local name = user_print_name(msg.from)
  ...
  if matches[1]:lower() == "بلیست" then -- Ban list !
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == 'عن' then -- /unban 
    if type(msg.reply_id)~="nil" and is_momod(msg) then
  ...
  if matches[1]:lower() == 'خفه شو' then
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      if is_admin(msg) then
  ...
  if matches[1]:lower() == 'اخراج کلی' then -- Global ban
    ...
    if msg.to.type == 'chat' then
      ...
      if string.match(targetuser, '^%d+$') then
        ...
        local member = string.gsub(matches[2], '@', '')
        local get_cmd = 'اخراج کلی'
        chat_info(receiver, username_id, {get_cmd=get_cmd, receiver=receiver, chat_id=msg.to.id, member=member})
      end
    end
  end
  if matches[1]:lower() == 'اخراج نیست' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
    if msg.to.type == 'chat' then
      if string.match(matches[2], '^%d+$') then
  ...
  if matches[1]:lower() == "لیست اخاجی ها" then -- Global ban list
    return banall_list()
  end
end
