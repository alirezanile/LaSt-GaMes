...
local function username_id(cb_extra, success, result)
   ...
   for k,v in pairs(result.members) do
      ...
      if vusername == member then
        ...
        member_id = v.id
        if mod_cmd == 'ادمین اصلیشو' then
            return admin_user_promote(receiver, member_username, member_id)
        elseif mod_cmd == 'ادمین اصلینی' then
...
function run(msg, matches)
    --vardump(msg)
    if matches[1] == 'گروه بساز' and matches[2] then
...
chat_info(receiver, returnids, {receiver=receiver})
	...
	if matches[2] then if data[tostring(matches[2])] then
		...
		if matches[1] == 'تغیر اسم' and is_admin(msg) then
		...
		if matches[1] == 'ادمین اصلیشو' then
			...
			local member = string.gsub(matches[2], "@", "")
				local mod_cmd = "ادمین اصلیشو"
				chat_info(receiver, username_id, {mod_cmd= mod_cmd, receiver=receiver, member=member})
			end
		end
		if matches[1] == 'ادمین اصلینی' then
		      if not is_momod(msg) then
            return 
        end
			if string.match(matches[2], '^%d+$') then
			...
			local member = string.gsub(matches[2], "@", "")
				local mod_cmd = "ادمین اصلینی"
				chat_info(receiver, username_id, {mod_cmd= mod_cmd, receiver=receiver, member=member})
			end
		end
		if matches[1] == 'لیستت' and matches[2] == 'ادمین ها' then
			return admin_list(msg)
		end
		if matches[1] == 'لیستت' and matches[2] == 'گروه ها' then
			group_list(msg)
...
return {
  patterns = {
    ...
    "^[!/](setting) (%d+)$",
        "^[!/](wholist)$",
        "^[!/](who)$",
    "(ادمین اصلیشو) (.*)$", -- sudoers only
    "(ادمین اصلینی) (.*)$", -- sudoers only
    "(لیستت) (.*)$",
        "([Ll]og)$",
        "^!!tgservice (.+)$",
  },
