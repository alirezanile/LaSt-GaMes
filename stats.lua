...
local function run(msg, matches)
  if matches[1]:lower() == 'بوت' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /teleseed ")
    return about
  end 
  if matches[1]:lower() == "لیست گروه ها" then
  ...
  if matches[1]:lower() == "امار" then
    ...
    if matches[2] == "بوت" then -- Put everything you like :)
...
return {
  patterns = {
    ...
    "(لیست امار) $",
    "(امار) (گروه) (%d+)",
    "(امار) (بوت)",-- Put everything you like :)
	"(بوت)"-- Put everything you like :)
    }, 
  run = run
}
